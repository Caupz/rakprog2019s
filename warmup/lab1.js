...
const generateRandomItemList = n => {
	...
	for(let i = 0; i < n; i++) {
		let prodName = "Toode "+i;
		let obj = {'name': prodName, 'cost': getRandomArbitrary(1,10000)};
		items.push(obj);
	}
...
const findMostExpensiveItem = items => {
	let expensiveItemValue = -99999999;
	let expensiveItem = null;
	
	for(let i = 0; i < items.length; i++) {
		if(items[i].cost > expensiveItemValue) {
			expensiveItemValue = items[i].cost;
...
const findCheapestItem = items => {
	let cheapestItemValue = 99999999;
	let cheapestItem = null;
	
	for(let i = 0; i < items.length; i++) {
		if(items[i].cost < cheapestItemValue) {
			cheapestItemValue = items[i].cost;
